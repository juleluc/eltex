...
ContactNode *add_contact_sorted(ContactNode *head, Contact new_contact) {
    ...
    if (head == NULL || strcmp(new_contact.name.FirstName, head->data.name.FirstName) < 0 ||
        ...
        new_node->next = head; // указатель на new узел
        if (head != NULL) {// если узел не пусто то указываем то голова должна указ на новый узел ч/з указ prev
            head->prev = new_node;
        }
        return new_node; 
    } 
    ...
    new_node->next = current->next; // new эл-т указ на след после current
...
ContactNode *load_contact(){
    ...
    while (read(fd, &contact, sizeof(Contact)) > 0){
        head = add_contact_sorted(head, contact);
    }
...
void save_contacts_to_file(ContactNode *head){
    ...
    ContactNode * current = head;
    while(current != NULL){
        write(fd, &(current->data), sizeof(Contact));
        current = current->next;
    }
